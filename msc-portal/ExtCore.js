...
(function(){
    var idSeed = 0,
        ...
        isEdge = check(/edge/),
        isOpera = check(/opera/),
        isChrome = !isEdge && check(/chrome/),
        isWebKit = !isEdge && check(/webkit/),
        isStreamy = check(/streamy/),
        isSafari = !isChrome && check(/safari/) && !isStreamy && !isEdge,
        isSafari2 = isSafari && check(/applewebkit\/4/), // unique to Safari 2
        isSafari3 = isSafari && check(/version\/3/),
        isSafari4 = isSafari && check(/version\/4/),
        isIE = !isOpera && check(/msie/),
        isIE11 = check(/trident\/7/),
        isIE7 = isIE && check(/msie 7/),
        isIE8 = isIE && check(/msie 8/),
        isIE6 = isIE && !isIE7 && !isIE8,
        isGecko = !isEdge && !isWebKit && !isIE11 && check(/gecko/),
...
function(el){
    ...
    // do something with the dom node
        ...
        isAir : isAir,
        /**
         * True if the detected platform is an iPad.
         * @type Boolean
         */
        isIPad : isIPad
    });
...
Ext.EventManager = function(){
    ...
    function initDocReady(){
        ...
        if (Ext.isGecko || Ext.isOpera || Ext.isIE11) {
            DOC.addEventListener(DOMCONTENTLOADED, fireDocReady, false);
	    // Ext.isIE does not cover IE11
        } else if (Ext.isIE){
...
(function(){
    ...
    var initExtCss = function(){
        ...
        if(!bd){ return false; }
        var cls = [' ',
                Ext.isIE ? "ext-ie " + (Ext.isIE7 ? 'ext-ie7' : (Ext.isIE8 ? 'ext-ie8' : (Ext.isIE9 ? 'ext-ie9' : (Ext.isIE10 ? 'ext-ie10': ''))))
        ...
        if(Ext.isSafari){
            ...
            if(Ext.isIPad){
                cls.push("ext-safari-ipad");
            }
        }else if(Ext.isStreamy){
            cls.push("ext-streamy");
...
el.un('click', this.handlerFn);
     ...
     * @return {String} The attribute value
     */
    getAttribute : (Ext.isIE8 || Ext.isIE9) ? function(name, ns){
        var d = this.dom,
...
Ext.Element.addMethods(function(){
	...
	return {
	     ...
	     * @param {Boolean} returnEl (optional) True to return a Ext.Element object instead of DOM node
	     * @return {HTMLElement|Ext.Element} The matching DOM node (or null if no match was found)
	     */
...
function() {
	...
	return {
		...
		/**
		 * Unwraps this element from its parent. Opposite  of the wrap() function.
		 * @return {Ext.Element}
		 */
		unwrap: function() {
			var unwrappedEl = this.parent().replaceWith(this);
			return unwrappedEl;
		},
...
Ext.Element.addMethods(function(){
    ...
    return {
        ...
        hasClass : function(className){
            ...
            return className && (' '+this.dom.className+' ').replace(rclass, ' ').indexOf(' '+className+' ') != -1;
        },
