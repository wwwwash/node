/* jquery.nicescroll
 ...
 Please note this has a few modifications against the original code.
 Anyway, this trash should be removed! Unify scrollbars pls :(
 */
	...
	var NiceScrollClass = function(myopt, me) {
		...
		this.init = function() {
			...
			if (!self.ispage || !cap.cantouch && !cap.isieold && !cap.isie9mobile) {
				...
				rail.width = Math.max(parseFloat(self.opt.cursorwidth), cursor.width());
				rail.css({
					width      : rail.width + 'px',
					zIndex     : self.zindex,
		...
		function execScrollWheel(e, hr, chkscroll) {
			...
			if (py) {
				if (self.opt.nativeparentscrolling && chkscroll && !self.ispage && !self.zoomactive) {
					...
					else if (self.getScrollTop() <= 0) {return true}
				}
				if (self.scrollmom) {
					self.scrollmom.stop();
				}
